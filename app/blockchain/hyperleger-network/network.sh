...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/broker/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    createOrg broker 7054

    infoln "Creating Org2 Identities"

    createOrg farmer 8054

    infoln "Creating Org3 Identities"

    createOrg cerealist 12054

    infoln "Creating Org4 Identities"

    createOrg mills 10054

    infoln "Creating Org5 Identities"

    createOrg baker 11054
...
function createChannel() {
  ...
  scripts/createChannel.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE $bft_true $CHANNEL_ID
}
